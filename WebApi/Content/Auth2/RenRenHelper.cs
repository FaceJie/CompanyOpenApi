...
namespace NFinal.OAuth
{
    public class renren_helper
    {
        ...
        {
            string send_url = "https://graph.renren.com/oauth/token?grant_type=authorization_code&client_id=" + config.oauth_app_id + "&client_secret=" + config.oauth_app_key + "&code=" + code + "&redirect_uri="
        ...
        public static Dictionary<string, object> get_info(OAuthConfig config,string access_token, string fields)
        {
            ...
            //发送并接受返回值
            string result = HttpUtility.HttpPost(send_url, param.ToString());
            if (result.Contains("error"))
            {
                return null;
            }
